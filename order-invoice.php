...
imagecopyresampled($tmp, $image, 0, 0, 0, 0, 300, $nh, 300, $nh);
...
<body style="background-color: #fff;font-family: 'Roboto', sans-serif">
	<div id="thank-you">
		...
		?>
			<div class="your-bill" style="min-height:11.693in;background:url('data:image/png;base64,<?= base64_encode($background) ?>') center no-repeat;background-size:40%;overflow:hidden">
						...
						<table class="table invoice-top" style="width:100%">
							<tr>
								<td width="25%" class="_inv_logo" style="padding:0 10px;vertical-align:top" align="middle">
									<img style="max-width:100%;max-height:100px" src="<?= Models::baseUrl('images/logo.png') ?>">
								</td>
								<td width="60%" style="padding:0 10px;vertical-align:top" class="tagline">
									<h2 class="company-name" style="margin:0;font-weight:700;font-size:28px;line-height:1em;text-transform:none"><?= COMPANY_NAME ?></h2>
									...
									<p class="company-address" style="margin:0 0 3px 0;text-align:left;color:#888;font-size:14px;margin-bottom:0;line-height:1.2em">
										<?= Models::getContactInformation('address'); ?>
									</p>
								...
								<td width="15%" class="qr" style="padding:0 10px;vertical-align:top;text-align:right">
									<img src="<?= 'https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=' . urlencode($SingleInvoiceUrl) . '&choe=UTF-8' ?>" style="width:100%;border:1px solid #e7e7e7" />
								</td>
							</tr>
						</table>
						<table class="table invoice-middle invoice-id" style="width:100%;margin-top:20px;margin-bottom:10px">
							<tr>
								<td style="padding:0 10px;vertical-align:middle">
									...
									<div class="separator"></div>
									<h3 style="margin-top: 10px;margin-bottom:5px">#<?= $OrderId ?></h3>
									<img style="width:250px;height:40px" src="<?= 'https://crm.dhakasolution.com/_ilmComm/barcode/?t' . urlencode(base64_encode($OrderId)) ?>" alt="<?= $OrderId ?>" />
								</td>
							</tr>
						</table>
						<table class="table invoice-middle" style="width:100%">
							<tr>
								<td class="invoice-info" width="50%" style="padding:0 10px;vertical-align:middle">
									<table border="0" style="width: 100%;width:auto;border-collapse:collapse;border-spacing:0">
										<tr>
											...
											<td style="vertical-align:top;font-size:13px;padding:1px 3px"><?= date("d-m-Y H:iA") ?> </td>
										</tr>
										<tr>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">Ordering Date</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">:</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px"><?= date("d-m-Y H:iA", strtotime($OrderInfo->getOrderDate())) ?> </td>
										</tr>
										<tr>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">Currency</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">:</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px"> <?= Models::curr() ?> </td>
										</tr>
										<tr>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">Payment Type</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">:</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px"><?= $OrderInfo->getPaymentType() ?></td>
										</tr>
										<tr>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">Issuer</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px">:</td>
											<td style="vertical-align:top;font-size:13px;padding:1px 3px"><?= htmlspecialchars($OrderInfo->getName() . ", " . $OrderInfo->getMobileNumber() . ", " . $OrderInfo->getEmail()) ?></td>
										</tr>
									</table>
								</td>
								<td class="invoice-bill-to" width="50%" style="padding:0 10px;vertical-align:middle">
									...
									<p style="margin:0 0 3px 0;text-align:left;margin-bottom:2px;font-size:13px;color:#000"><strong><?= htmlspecialchars($OrderInfo->getShippingName()) ?></strong></p>
									<p style="margin:0 0 3px 0;text-align:left;margin-bottom:2px;font-size:13px;color:#000"><?= htmlspecialchars($OrderInfo->getShippingAddress()) ?></p>
									<p style="margin:0 0 3px 0;text-align:left;margin-bottom:2px;font-size:13px;color:#000">Delivery Location: <?= htmlspecialchars($OrderInfo->getDeliveryLocation()) ?></p>
									<p style="margin:0 0 3px 0;text-align:left;margin-bottom:2px;font-size:13px;color:#000"><?= htmlspecialchars($OrderInfo->getShippingPhone()) ?></p>
									<p style="margin:0 0 3px 0;text-align:left;margin-bottom:2px;font-size:13px;color:#000"><?= htmlspecialchars($OrderInfo->getEmail()) ?></p>
								</td>
							</tr>
						</table>
						<div class="invoice-table">
								...
								<tbody>
									...
									?>
										<tr style="border-bottom:1px solid #ccc;color:#333;font-weight:500">
											...
											<td style="padding:10px;vertical-align:middle"><?= $Prs["q"] ?></td>
											<td style="padding:10px;vertical-align:middle">
											...
											<td style="padding:10px;vertical-align:middle"><?= Models::curr($Sp->getPrice() * $Prs["q"]) ?></td>
										</tr>
									<?php
									endforeach;
							...
							<table class="itemTotal" border="0" style="width:35%;border-collapse:collapse;border-spacing:0;margin-top:10px;font-size:14px;float:right;color:#333">
								<tr>
									...
									<td style="padding:3px;vertical-align:middle"><?= Models::curr($Total) ?></td>
								</tr>
								<tr>
									<td style="padding:3px;vertical-align:middle">Delivery Cost</td>
									<td style="padding:3px;vertical-align:middle"><?= Models::curr($DCost) ?></td>
								</tr>

								<?php if ($CouponDiscount) : ?>
									<tr>
										<td style="padding:3px;vertical-align:middle">Coupon Discount</td>
										<td style="padding:3px;vertical-align:middle"><?= Models::curr($CouponDiscount) ?></td>
									</tr>
								<?php endif; ?>
								...
								<tr class="subtotal" style="color:#000;border-top:2px dotted #aaa;font-size:16px">
									...
									<td style="padding:3px;vertical-align:middle"><?= Models::curr($SubTotal) ?></td>
								</tr>
							</table>
							...
							<div class="payment-info" style="color:#888;font-size:12px;margin-top:20px;width:100%;font-weight:400">
								<p style="margin:0 0 3px 0">
									Payment Type: <?= $OrderInfo->getPaymentType() ?>
									<?php if ($CouponDiscount) : ?>
										, Used coupon: <?= $CpInfo->getCouponCode() ?>
									<?php endif; ?>
								</p>
								<p style="margin:0 0 3px 0">Payment TrxnId: <?= $OrderInfo->getPaymentTrxnId() ?></p>
								<img style="width:400px;height:60px" src="<?= 'https://crm.dhakasolution.com/_ilmComm/barcode/?t=' . urlencode(base64_encode($OrderInfo->getPaymentTrxnId())) ?>" alt="<?= $OrderInfo->getPaymentTrxnId()  ?>" />
								<p style="font-size:11px">
									Your complete satisfaction<br /><br />
									We want to make sure that your are completely satisfied with <?php echo COMPANY_NAME ?><br /><br />
									Delivery: If for any reason you are not, then please advise the Customer Services Team Member at the door and they will ensure that any issues will be resolve for you.<br /><br />
									If there are any questions you'd like to ask, either about your order or any aspect of the <?php echo COMPANY_NAME ?> service, Please call us on<br />
									<?php echo Models::getContactInformation('mobile1') ?>, or e-mail us at <?php echo Models::getContactInformation('email') ?>. We are open seven days a week.
								</p>
							</div>
						</div>
					</div>
