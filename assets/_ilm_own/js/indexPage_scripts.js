_ilm_home_page = {
	init: function () {
		...
		$('.bc-cat-tab a').click(function () {
			var $env = $(this),
				$bcElem = $env.closest('.bc-single'),
				CatData = $bcElem.data(),
				SortVal = $env.data("sv"),
				ajaxData = {
					homeProductSorting: 1,
					c: CatData.catid,
					cn: CatData.catname,
					v: SortVal
				};

			$bcElem.find('.bc-cat-tab .active').removeClass('active');
			$env.addClass('active');

			ajaxPost(ajaxData, function (data) {
				$bcElem.find('.grid-row').html(data);
			})
		});
		...
		if ($("#featured-products").length) {
			feature_slider = new $JssorSlider$("featured-products", featured_products_options);
		}
