...
?>
	...
	<section class="main-body">
		<div class="container">
			...
			?>	
				<div class="order-track" style="background-image:url('<?= $StatusImage ?>');text-align:left">
					<h3><?= $OdrStatus['text'] ?></h3>
					<div class="track-timeline">
						<p>Timeline</p>
						<ul class="timeline-ul">
							<li>
								<span><?= date('F j, Y h:iA', strtotime($OdrStatus['time'])) ?></span>
								Your placed order #<?= $TrackId ?>.
							</li>
						<?php
						...
						<?php if(($OdrStatus['status'] > 1) && (!$MT['company_name'] && $MT['processed_on'])): ?>	
							<li>
								<span><?= date('F j, Y h:iA', strtotime($MT['processed_on'])) ?></span>
								<?= COMPANY_NAME ?> processed your order.
							</li>
						<?php endif; ?>
						
						<?php if($OdrStatus['status'] > 2): ?>
							<li>
								<span><?= date('F j, Y h:iA', strtotime($MT['shipped_on'])) ?></span>
								<?= $MT['company_name'] ?: COMPANY_NAME ?> shipped Products to <?= $MT['company_name'] ? COMPANY_NAME : 'your Logistic partner' ?>.
							</li>
						<?php endif; ?>
							
						<?php if(($OdrStatus['status'] > 3) && (!$MT['company_name'] && $MT['delivered_on'])): ?>
							<li>
								<span><?= date('F j, Y h:iA', strtotime($MT['delivered_on'])) ?></span>
						...
						<?php if(($OdrStatus['status'] > 5) && (!$MT['company_name'] && $MT['returned_on'])): ?>
							<li>
								<span><?= date('F j, Y h:iA', strtotime($MT['returned_on'])) ?></span>
								<?= COMPANY_NAME ?> returned your order.
							</li>
						<?php endif; ?>
