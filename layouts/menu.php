...
?>
	<li class="dropdown"> 
		<a href="<?= $Cat->getHref() ?>">
			<span><img src="<?= Models::baseUrl('images/category-slides/' . Models::restyleUrl($Cat->Mainc) . '-1.png?rand=' . rand()) ?>"></span>
			<?= htmlspecialchars($Cat->Mainc) ?> <i class="fa fa-angle-down"></i>
		</a>
		<ul role="menu" class="sub-menu<?= $submenu_addClass ?>">
			<div class="col-xs-12 sub-cols view-all hidden-md hidden-lg">
				<a href="<?= $Cat->getHref() ?>">
					View All <?= htmlspecialchars($Cat->Mainc) ?> &nbsp;<i class="fa fa-long-arrow-right"></i>
				</a>
			</div>
